...
void printArray(MinPQ *pq,EdgeList *adjInfo,int n,int *parent,double *fringeWgt,int *status);
...
int from; // we use this variable to store the first number of every lines, it signify the start vertex 

/**Reade the line and allocate the number as from to weight */
EdgeInfo parseEdgeU(char line[]){
	EdgeInfo newE;
	int numWords;
	numWords=sscanf(line,"%d %d %lf",&from, &newE.to, &newE.wgt);
...
void loadWgtGraph(char *line, EdgeList *adjVertices,int task)
{
	EdgeInfo e;
	e=parseEdgeU(line);
	adjVertices[from]=edgeCons(e,adjVertices[from]);
	/*task==1 is for prim's algorithm, so this part would load unidrected graph, otherwise load directed graph */
	if(task==1){
	int instant=from;
	from=e.to;
	e.to=instant;
...
void greedyTree(EdgeList *adjInfo,int n,int s,int *parent,double *fringeWgt,int task){
	...
	printf("------------------------------------------------\n");
	printArray(pq,adjInfo,n,parent,fringeWgt,status);
}
...
void printArray(MinPQ *pq,EdgeList *adjInfo,int n,int *parent,double *fringeWgt,int *status){
	...
	if(pq->status[i]==INTREE)
		printf("tree\n");
	else if(pq->status[i]==FRINGE)
		printf("fringe\n");
	else 
		printf("unseen\n");
	}
	...
	EdgeList *oriGraph=calloc(n+1,sizeof(EdgeList));   
        oriGraph=initEdgesP(n);
      	...
      	while (fgets(line, sizeof(line), fp)){
			loadWgtGraph(line,oriGraph,task); //load unidrected or directed weighted graph based on the comment
        }     
